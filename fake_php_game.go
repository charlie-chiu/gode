...
func (g FakePhpGame) OnTakeMachine(uid UserID) []byte {
	r := &response{
...
func (g FakePhpGame) OnLoadInfo(uid UserID, gc GameCode) []byte {
...
func (g FakePhpGame) OnGetMachineDetail(uid UserID, gc GameCode) []byte {
	r := &response{
...
func (g FakePhpGame) BeginGame(sid SessionID, gc GameCode, betInfo string) []byte {
	rand.Seed(time.Now().UnixNano())
	if rand.Intn(10)%2 == 0 {
		return []byte(beginGameResult1)
	} else {
		return []byte(beginGameResult2)
	}
}

func (g FakePhpGame) OnCreditExchange(sid SessionID, gc GameCode, bb string, credit int) []byte {
	msg := `{"action":"onCreditExchange","result":{"data":{"Balance":99949999.99,"BetBase":"1:1","Credit":100,"event":true},"event":true}}`
	return []byte(msg)
}

func (g FakePhpGame) OnBalanceExchange(uid UserID, hid HallID, gc GameCode) []byte {
...
func (g FakePhpGame) OnLeaveMachine(uid UserID, hid HallID, gameCode GameCode) []byte {
	panic("implement me")
}
